package com.lomicron.eu4.serializers
...
object ClausewitzSerializer {
  ...
  def serializeHistory[H <: History[H, S, E], S <: HistState[S, E], E <: HistEvent]
  (a: History[H, S, E], modifyEvent: ObjectNode => ObjectNode = identity): String = {
    val init = Option(a.init).flatMap(toObjectNode).map(modifyEvent).getOrElse(objectNode)
    val events = a.events
      .flatMap(e => for {
        json <- toObjectNode(e)
        date <- e.date
      } yield (date, json.removeEx(Fields.date)))
      .foldLeft(objectNode)((acc, t) => acc.setEx(t._1.toString, modifyEvent(t._2)))
  ...
  def recSerialize(node: ObjectNode, depth: Int = 0, options: Set[SerializationOption] = Set.empty): Seq[String] = {
    ...
    node.entries().flatMap(e => {
      ...
      v match {
        case o: ObjectNode =>
          val serializedObj = recSerialize(o, depth + 1, options)
