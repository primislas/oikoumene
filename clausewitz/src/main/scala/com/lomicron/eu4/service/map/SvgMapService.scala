...
import com.lomicron.eu4.service.svg.SvgElements._
import com.lomicron.eu4.service.svg._
...
case class SvgMapService(repos: RepositoryFactory, settings: SvgMapSettings = SvgMapSettings.default) extends LazyLogging {
  ...
  def classesOfProvince(pId: Int, mapMode: String): Seq[String] =
    repos.provinces.find(pId).map(SvgMapClasses.ofProvince(_, mapMode)).getOrElse(Seq.empty)
  ...
  def borderBetweenProvIds(p1: Option[Int], p2: Option[Int]): String =
    borderBetweenProvs(
      p1.flatMap(repos.provinces.find(_)),
      p2.flatMap(repos.provinces.find(_))
    )
