package com.lomicron.eu4.service.government

import com.lomicron.eu4.model.government.TechLevel
import com.lomicron.eu4.model.politics.{Tag, TagState}
...
case class TagService(repos: RepositoryFactory) {
  ...
  def init(t: Tag): Tag = {
    ...
    val gov = its.government
      .flatMap(repos.governments.find(_))
      .map(_.basicReform)
      .flatMap(repos.governmentReforms.find(_))
