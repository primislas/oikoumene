...
case class InMemoryDiplomacyRepository()
  ...
  override def create(entity: DiploRelation): DiploRelation =
    addRelsByTag(super.create(entity))

  override def tagRelations(tag: String): Seq[DiploRelation] =
    relsByTag.get(tag).map(_.toSeq).getOrElse(Seq.empty).flatMap(find)

  override def remove(key: Int): Option[DiploRelation] = super
    .remove(key)
