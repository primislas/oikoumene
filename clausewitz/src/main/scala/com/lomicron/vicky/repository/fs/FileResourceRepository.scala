...
case class FileResourceRepository(settings: GameFilesSettings)
  ...
  val productionTypesFile = "common/production_types.txt"
  ...
  def getCountryConfigs(filesByTag: Map[String, GameFile]): Seq[TagFileConf] = {
    ...
    filesByTag
      .map(e => {
        ...
        TagFileConf(tag, tagConfWithFullPath, hist)
      })
      .toSeq
  }
  ...
  override def getProductionTypes: Seq[GameFile] =
    readGameFile(productionTypesFile).toSeq
